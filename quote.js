...
$("input[name='productLine']").on("click", function(){
    ...
    {
       $("#unit_price").val(productLineType)
       $("#num_of_elevators").val(getNumberOfElavators());
       var totalPriceforElevator=getNumberOfElavators()*parseInt(productLineType);
       $("#total_price").val(totalPriceforElevator);
...
function getNumberOfElavators()
{
    if(isCommercial()){
      ...
      if(numberOfFloors<=0|| numberOfOccupantsPerFloor<=0|| numberOfBasements<=0){
          return;
      }
        var totalNumerOfOccupants= numberOfOccupantsPerFloor* (numberOfFloors+numberOfBasements);
        ...
        var numberOfElavatorsPerColumn= Math.ceil(numberOfRequiredElavators/numberOfColumns);
